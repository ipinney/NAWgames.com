...
PARTS = [
    ...
    ('3D', 'Whole setup, assembled', 'done', [('3D model', 'ms2000-3d.html'), ('Turret part by part', 'ms2000-turret.html')],
     'Everything together: the turret, the black backdrop on its feet, the dowel with the pendulum pivot, the stuffed mosquito with its glowing pod hanging 3 ft from the camera, the wand, the ribbon cable, and the laser beam. Zoom to the turret or the mosquito with one tap.'),
    ...
    ('P1', 'Base shell', 'done', [('3D viewer', 'ms2000-p1-base-shell.html'), ('STL', 'stl/ms2000-p1-base-shell.stl')],
     '142 x 133 x 67, Rev B (MOC-004). Front window for the micro:bit LEDs, lift-out slot, USB-C charge port and power switch window on the right, speaker grille and laser arm switch on the back. About 169 g.'),
    ('P1', 'Floor plate', 'done', [('3D viewer', 'ms2000-p1-floor-plate.html'), ('STL', 'stl/ms2000-p1-floor-plate.stl')],
     'Rev B (MOC-004). Board posts, power bank cradle with strap slots, and the stand the power switch sits on. Four screws up into the shell. Board hole spacing is from a photo. About 76 g.'),
