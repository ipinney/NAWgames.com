...
- Sep 16: turret.tpl.html and setup.tpl.html restyled like Dusty's components viewer: bottom card (name, specs, note), one side-scrolling tool row (focus, views, toggles, Print files), one side-scrolling chip row. Turret chips (Whole turret, Printed, Bought) highlight a part and show its note in the card. make_hero/make_og/make_guide_shots reserve space with #card now.
