"""Plate STLs -> Flash Studio project files (.3mf) + gcode for the Adventurer 5M, adapted from dusty-chassis/orca.
Run on Vultr: python3 orca/build3mf.py  (needs plates.py output in /opt/ffstudio/work/ms2000)"""
import json, os, subprocess, zipfile, re, glob, shutil, sys
W='/opt/ffstudio/work/ms2000'
S='/opt/ffstudio/work'  # flattened Flashforge presets (see dusty-chassis/orca/flatten.py)
PLATES={p['n']:(p['slug'],p['title']) for p in json.load(open(f'{W}/plates.json'))}
COMMON={'wall_loops':'3','top_shell_layers':'5','bottom_shell_layers':'4','sparse_infill_density':'15%',
...
PER_OBJ={'pin':{'brim_type':'no_brim'},'cable_clips':{'brim_type':'no_brim'}}
...
def rewrite(src,dst,n,slow):
    ...
    for it in zin.infolist():
        ...
        if it.filename=='Metadata/project_settings.config':
            ...
            data=data.replace(b'<metadata name="Title"></metadata>',f'<metadata name="Title">MS-2000 plate {n}: {PLATES[n][1]}</metadata>'.encode())
            data=data.replace(b'<metadata name="Designer"></metadata>',b'<metadata name="Designer">nawgames.com/projects/addie/mosquito-turret</metadata>')
        zout.writestr(it,data)
    zout.close()

os.makedirs(f'{W}/out',exist_ok=True)
m,p,f=f'{S}/m.json',f'{S}/p.json',f'{S}/f.json'
summary={}
for n,(slug,title) in PLATES.items():
    stls=' '.join(sorted(glob.glob(f'{W}/plate{n}/*.stl')))
    ...
    final=f'{W}/out/ms2000-plate-{n}-{slug}.3mf'
    rewrite(f'{raw}/p{n}.3mf',final,n,n in (3,))
    ...
    shutil.copy(g[0], f'{W}/out/ms2000-plate-{n}-{slug}.gcode') if g else None
