...
C['Battery holder'] = retext(C['Battery holder'],
    what='A black box for four AA batteries, with a little switch and a wire ending in a round plug.',
    does='powers everything. It sits flat under the deck, right between the wheels and the caster, where its weight keeps Dusty steady.',
    spec='SWITCH OFF WHILE YOU BUILD')
...
extra = [
    ...
    newcard('M2 screws and nuts', svg_screw,
        ...
        'ABOUT 20 SCREWS &middot; 6 NUTS &middot; PHILLIPS #0'),
    ...
    newcard('Power splitter', svg_split,
        'A short Y cable that turns one battery plug into two, plus a little adapter with screw terminals on the end.',
        'lets the battery feed the moto:bit and the brush motor at the same time, with no cutting or soldering.',
        '2.1 MM PLUG &middot; GROWN-UP CONNECTS IT'),
]

order = ['micro:bit v2', 'Motor driver board', 'N20 gear motor &times;2', 'Wheels &times;2', 'Ball caster',
         'Cliff sensors &times;2', 'Whisker switch', 'Brush motor', 'Battery holder']
...
steps.append(step(4, 'Add the deck, the brain and the power', 'Sat Sep 19 &middot; about 45 minutes',
    ...
    reused_explain + '\n' +
    explain('Why the battery goes in the middle',
        'The batteries are the heaviest thing on Dusty, about 92 grams. Sitting between the wheels and the caster, their weight presses the wheels into the table so they grip, and keeps the front from tipping.') + '\n' +
    see3d('4xAA holder') + '\n' +
    done('Flipping the switch lights the micro:bit, and Dusty still sits flat with everything on board.')))
  ...
  <div class="card">
    ...
    <div class="table-wrap">
      <table>
        ...
        <tbody>
          <tr><td>Nothing happens at all</td><td>Power</td><td>Switch on? Batteries the right way round? Plugs pushed fully in? Try fresh batteries.</td></tr>
          ...
          <tr><td>Crumbs fly forward</td><td>Brush spins the wrong way</td><td>Battery off, swap the two wires on the brush motor tabs.</td></tr>
          <tr><td>Gets slower as you test</td><td>Batteries draining</td><td>Normal. Fresh batteries for every measured set, so the test is fair.</td></tr>
    ...
    <div class="table-wrap">
      <table class="screws">
        ...
        <tbody>
          ...
          <tr><td>Deck</td><td>M2 &times; 8</td><td>4</td><td>Down into the posts</td></tr>
          ...
          <tr><td><b>Total</b></td><td></td><td><b>19</b></td><td>14 &times; 8 mm, 3 &times; 6 mm, 2 &times; 10 mm, plus 6 nuts. Buy a small M2 kit; spares get lost.</td></tr>
        </tbody>
      </table>
    </div>
  </div>'''
