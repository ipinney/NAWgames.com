...
PER_OBJ={'fitcheck':{'brim_type':'no_brim'}}
...
def rewrite(src,dst,n,slow):
    ...
    for it in zin.infolist():
        ...
        if it.filename=='Metadata/project_settings.config':
            ...
            data=data.replace(b'<metadata name="Title"></metadata>',f'<metadata name="Title">Flying Mosquito plate {n}: {PLATES[n][1]}</metadata>'.encode())
            data=data.replace(b'<metadata name="Designer"></metadata>',b'<metadata name="Designer">nawgames.com/projects/addie/flying-mosquito</metadata>')
...
for n,(slug,title) in PLATES.items():
    ...
    final=f'{W}/out/flymo-plate-{n}-{slug}.3mf'
    rewrite(f'{raw}/p{n}.3mf',final,n,n in (2,3,4))
    ...
    shutil.copy(g[0], f'{W}/out/flymo-plate-{n}-{slug}.gcode') if g else None
